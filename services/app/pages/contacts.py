...
from models.model import get_db, Contact
        ...
        with get_session() as session:
            with open(filepath, 'r', encoding='utf-8') as csv_file:
                ...
                for row in reader:
                    ...
                    contact = session.query(Contact).filter(Contact.name == name, Contact.phone == phone).first()                    
                    if contact:
                ...
                session.query(Contact).filter(Contact.updated_at < upload_time).delete()
                session.commit()
